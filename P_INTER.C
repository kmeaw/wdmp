...
//-----------------------------------------------------------------------------
//
// $Id: p_inter.c,v 1.2 1997/12/29 19:51:08 pekangas Exp $
//
...
const char
p_inter_rcsid[] = "$Id: p_inter.c,v 1.2 1997/12/29 19:51:08 pekangas Exp $";
  ...
  boolean	dropped )
{
	...
	if (player == &players[g_cSoundedPlayer])
  ...
  mobj_t*	target )
{
	...
	if (target->player == &players[g_cCurrentPlayerForCmd]
	    && automapactive[g_cCurrentPlayerForCmd])
	{
  ...
  int 		damage )
{
	...
	if (player == &players[g_cCurrentPlayerForCmd])
