...
/*  write(audio_fd, mixbuffer, SAMPLECOUNT*BUFMUL);*/   // Laissez comme ça !!
}
...
extern DWORD win[MAXPLAYERS];  /* window handle from win32video.c (actually HWND) */ // MRMP
...
/*
    ...
    nosound = !!M_CheckParm("-nosound");
        ...
        // MR010798
        g_bSharedSounds= !!M_CheckParm("-sharesound");
        i = M_CheckParm ("-Splayer");
    if (i && i < myargc-1)
    {
                g_cSoundedPlayer=myargv[i+1][0]-'0'-1;
    }
        // End MR300698
    ...
    // We'll need to initialize graphics here so that we can get a window handle for DirectSound:
    I_InitGraphics();
    
    if ( !wavonly )
    {
        MIDASsetOption(MIDAS_OPTION_DSOUND_HWND, win[0]);               // MRMP
        if ( primarysound )
