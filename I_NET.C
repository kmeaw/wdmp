...
// MR (2 players)
void fn_vSendPacket(doomdata_t	*sw,short wDL);
...
//
	fn_vSendPacket(&sw,doomcom->datalength);	// MR0698
	// End MR
...
//    doomcom = malloc (sizeof (*doomcom) );
    for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
		memset (&doomcomMP[cPlayer], 0, sizeof(*doomcom) );
    ...
    {
	    for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
		{	doomcomMP[cPlayer].ticdup = myargv[i+1][0]-'0';
			if (doomcomMP[cPlayer].ticdup < 1)
				doomcomMP[cPlayer].ticdup = 1;
			if (doomcomMP[cPlayer].ticdup > 9)
				doomcomMP[cPlayer].ticdup = 9;
		}
    }
    else
	    for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
    ...
    if (M_CheckParm ("-extratic"))
		for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
			doomcomMP[cPlayer].extratics = 1;
    else
		for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
			doomcomMP[cPlayer].extratics = 0;
    ...
    for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
		doomcomMP[cPlayer].consoleplayer = cPlayer;
...
//    }
	...
	for (cPlayer=0;cPlayer<C_PLAYERNUMBER;cPlayer++)
	{	doomcomMP[cPlayer].numnodes = 2;

	    doomcomMP[cPlayer].id = DOOMCOM_ID;
		doomcomMP[cPlayer].numplayers = doomcomMP[cPlayer].numnodes;
	}
