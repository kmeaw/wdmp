...
//-----------------------------------------------------------------------------
//
// $Id: wi_stuff.c,v 1.3 1998/01/04 19:29:08 pekangas Exp $
//
...
//
// $Log: wi_stuff.c,v $
// Revision 1.3  1998/01/04 19:29:08  pekangas
...
const char
wi_stuff_rcsid[] = "$Id: wi_stuff.c,v 1.3 1998/01/04 19:29:08 pekangas Exp $";
...
// wbs->pnum
static int		me[MAXPLAYERS];	// MRFin

 // specifies current state
static stateenum_t	state[MAXPLAYERS];
...
void WI_updateAnimatedBack(void)
{
	...
	if (bcnt == a->nexttic)
	{
		...
		if (!(state[g_cCurrentPlayerForDsp] == StatCount && i == 7)
		    && wbs->next == a->data1)
		{
		    a->ctr++;
...
{
    state[g_cCurrentPlayerForDsp] = NoState;
    acceleratestage = 0;
    cnt = 10;
}
...
{
    state[g_cCurrentPlayerForDsp] = ShowNextLoc;
    acceleratestage = 0;
...
void WI_initDeathmatchStats(void)
{
    ...
    state[g_cCurrentPlayerForDsp] = StatCount;
    acceleratestage = 0;
    dm_state = 1;
...
void WI_drawDeathmatchStats(void)
{
	...
	if (playeringame[i])
	{
	    ...
	    if (i == me[g_cCurrentPlayerForDsp])
	    {
		V_DrawPatchInDirect(x-SHORT(p[i]->width)/2,
			    DM_MATRIXY - WI_SPACINGY,
			    FB,
...
void WI_updateStats(void)
{
    ...
    {
	acceleratestage = 0;
	cnt_kills[0] = (plrs[me[g_cCurrentPlayerForDsp]].skills * 100) / wbs->maxkills;
	cnt_items[0] = (plrs[me[g_cCurrentPlayerForDsp]].sitems * 100) / wbs->maxitems;
	cnt_secret[0] = (plrs[me[g_cCurrentPlayerForDsp]].ssecret * 100) / wbs->maxsecret;
	cnt_time = plrs[me[g_cCurrentPlayerForDsp]].stime / TICRATE;
	...
	if (cnt_kills[0] >= (plrs[me[g_cCurrentPlayerForDsp]].skills * 100) / wbs->maxkills)
	{
	    cnt_kills[0] = (plrs[me[g_cCurrentPlayerForDsp]].skills * 100) / wbs->maxkills;
	    S_StartSound(0, sfx_barexp);
	    sp_state++;
	...
	if (cnt_items[0] >= (plrs[me[g_cCurrentPlayerForDsp]].sitems * 100) / wbs->maxitems)
	{
	    cnt_items[0] = (plrs[me[g_cCurrentPlayerForDsp]].sitems * 100) / wbs->maxitems;
	    S_StartSound(0, sfx_barexp);
	    sp_state++;
	...
	if (cnt_secret[0] >= (plrs[me[g_cCurrentPlayerForDsp]].ssecret * 100) / wbs->maxsecret)
	{
	    cnt_secret[0] = (plrs[me[g_cCurrentPlayerForDsp]].ssecret * 100) / wbs->maxsecret;
	    S_StartSound(0, sfx_barexp);
	    sp_state++;
	...
	if (cnt_time >= plrs[me[g_cCurrentPlayerForDsp]].stime / TICRATE)
	    cnt_time = plrs[me[g_cCurrentPlayerForDsp]].stime / TICRATE;
...
void WI_Ticker(void)
{
    ...
    switch (state[g_cCurrentPlayerForDsp])
    {
      case StatCount:
...
void WI_initVariables(wbstartstruct_t* wbstartstruct)
{
    ...
    cnt = bcnt = 0;
    firstrefresh = 1;
    me[g_cCurrentPlayerForDsp] = wbs->pnum;
    plrs = wbs->plyr;
