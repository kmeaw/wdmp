...
byte*				screens[5];	
byte*				screensMP[5*(MAXPLAYERS+1)];	
int	usegamma;

// MRMP
#define MAXHEIGHT			832		// R_draw1.c

extern byte*		ylookupMP[MAXPLAYERS][MAXHEIGHT]; 
extern byte*		*ylookup; 

// MRMP
void fn_vSetCurrentScreen(char cPlayer)
{ int i;

	displayplayer = cPlayer;
	ylookup=ylookupMP[cPlayer];
	for (i=0;i<5;i++)
		screens[i]=screensMP[5*cPlayer+i];
	//screens=&screensMP[5*cPlayer];
	//memcpy(screens,screensMP[5*cPlayer],
}
...
extern boolean g_bAutosize;	// MR280798
...
#ifndef WATWIN31
  else if ((C_PLAYERNUMBER>2)||(!g_bAutosize))	// MR280798
#endif
    SCREENWIDTH=320;
#ifndef WATWIN31
  else
